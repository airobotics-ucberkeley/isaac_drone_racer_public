...
class DroneRacerSceneCfg(InteractiveSceneCfg):
    ...
    track: RigidObjectCollectionCfg = generate_track(
        track_config={
            "1": {"pos": (14.0, 34.0, 0.0), "yaw": gate_transform((1 / 2) * torch.pi)},
            "2": {"pos": (20.0, 30.0, 0.0), "yaw": gate_transform((1 / 4) * torch.pi)},
            "3": {"pos": (21.0, 22.0, 0.0), "yaw": gate_transform((1 / 6) * torch.pi)},
            "4": {"pos": (21.0, 14.0, 0.5), "yaw": gate_transform(- (1 / 6) * torch.pi)},
            "5": {"pos": (19.0, 6.0, 0.0), "yaw": gate_transform((4 / 9) * torch.pi)},
            "6": {"pos": (14.0, 14.0, 0.0), "yaw": gate_transform((1 / 2) * torch.pi)},
            "7": {"pos": (9.0, 6.0, 0.0), "yaw":gate_transform( - (1 / 10) * torch.pi)},
            "8": {"pos": (8.0, 14.0, 0.0), "yaw": gate_transform((1 / 10) * torch.pi)},
            "9": {"pos": (6.0, 22.0, 0.0), "yaw": gate_transform(- (1 / 10) * torch.pi)},
            "10": {"pos": (8.0, 30.0, 0.0), "yaw": gate_transform(- (1 / 4) * torch.pi)},
        }
    )
    ...
    robot: ArticulationCfg = FIVE_IN_DRONE.replace(
        ...
        init_state=FIVE_IN_DRONE.init_state.replace(
            pos=(10.0, 34.0, 0.0)  # new position
        )
    )
