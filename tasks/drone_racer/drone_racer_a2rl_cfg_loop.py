...
class DroneRacerSceneCfg(InteractiveSceneCfg):
    ...
    track: RigidObjectCollectionCfg = generate_track(
        track_config={
            ...
            "6": {"pos": (8.0 - init_x, 14.0 - init_y, 0.0), "yaw": gate_transform((1 / 10) * torch.pi)},
            "7": {"pos": (6.0 - init_x, 22.0 - init_y, 0.0), "yaw": gate_transform(- (1 / 10) * torch.pi)},
            "8": {"pos": (8.0 - init_x, 30.0 - init_y, 0.0), "yaw": gate_transform(- (1 / 4) * torch.pi)},
        }
    )
